...
(*
Lemma fv_wf_term: forall t x e, wf_term t -> In x (fv_term t) -> In x (dom e).
Proof.
  induction 1; simpl; intros; auto. inversion H; subst; auto. exists ((x, (Tparam x)) :: nil) . simpl; auto. tauto.
  apply in_app_or in H1; destruct H1 as [H1 | H1].
  destruct (fresh_name TERM (x :: fv_term a ++ dom e)) as [y [F K]].
  apply H0 with y; auto. 
  assert (In x (dom ((y, t) :: e))). destruct ( H0 y); eauto. 
  apply fv_term_freshen_term. auto. 
  destruct H.  (H y); auto. simpl dom in H4. eauto.

tauto.  induction H. simpl in H0; subst; auto. auto. congruence.  discriminate.  tauto. contradiction.  induction 1; simpl; intros. admit. 
...
(*
  ...
  apply wf_type_vars_below_0 with (e:=e) (x:=x) ; induction t; auto; trsplit.
  simpl. left; auto. split; [apply IHt1 | apply IHt2]; intros; auto. 
  ...
  apply terms_var apply wf_type_vars_below_0 with (x:=x) (e:=e).
  induction t; auto; simpl. left; auto. 

  
  (* apply IHt1; auto; intros. simpl. ; auto.  *)


(* admit.  (* something's wrong  *) *)
  destruct (fresh_name TERM (fv_term a ++ dom e)) as [x [F K]].
  apply term_vars_below_vsubst_term with (Param x). apply H0; auto. 
Qed.
*)
...
Proof.
  ...
  - destruct (notIn_app name x (fv_type t) (fv_term a1) H). rewrite IHa1; auto. 
...
Lemma fresh_freshen_term:
  forall x e a y,
  wf_term (freshen_term a x) -> ~In y (dom e) -> x <> y ->
  ~In y (fv_term a).
Proof.
  intros. intro F. assert (In y (x :: dom e)).
  generalize (fv_term_freshen_term y a x F); intros. apply H0. intros; auto.
generalize (wfct simpl in H. 
  
  red; intro.
  assert (In y (x :: dom e)). generalize (fv_term_freshen_term y a x H2). intros; auto.  apply in_cons. contradiction. 
